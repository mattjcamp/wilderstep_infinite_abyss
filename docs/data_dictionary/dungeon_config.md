...
Distinct from Map because dungeons are not authored — they're procedurally generated. A Dungeon Config holds the generator's input parameters (number of levels, torch density, difficulty curve, monster and loot pools, etc.) rather than tile data. The generated dungeon itself becomes a set of Map instances at runtime, stored in the Game (not in the Module).
