...
`web/public/modules/default/counters.json` — real data drives the schema.
...
- **References [Item](item.md) by id.** Counter `items[]` was the first model to flip from name to id; Recipe `reagents`, Spawn `loot`, Party `inventory[].item`, weapon `ammo`, and Character `equipped.*` + `inventory[].item` followed. No name-based Item references remain anywhere in v2.
