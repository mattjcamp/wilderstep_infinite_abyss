...
- **No `abilities` or `known_spells` arrays yet.** Class-granted abilities and spells live on [Character Class](character_class.md) and on [Spell](spell.md)'s `allowable_classes`. If a character has unique acquired abilities (e.g. learned from a quest, granted by an artifact), a future `granted_effects` / `granted_spells` array on Character would be the right home.
