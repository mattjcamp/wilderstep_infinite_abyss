...
An **authored** multi-level dungeon — a hand-painted adventure with named floors the party descends through. Each Dungeon record names the dungeon and lists its [Dungeon Level](dungeon_level.md) records in order; each Dungeon Level points at a [Map](map.md) for its tile grid.
...
The "Used?" column reflects the v2 TypeScript implementation under `web/`. The codebase is in its early stages, so every field is currently `TBD` until it is wired up. This model is a **stub** — fields beyond `id`, `name`, and `levels[]` are open.
...
| `levels` | string[] | yes | Ordered list of [Dungeon Level](dungeon_level.md) ids; index 0 is the entrance floor, deeper floors follow. | TBD |

## Cross-references to other models

- `levels[]` → [Dungeon Level](dungeon_level.md) ids
...
- **Schema is a stub.** Only the three field positions are decided. Likely future additions: party-entry coordinates per level (so descending stairs land somewhere specific), per-dungeon ambient lighting, per-dungeon encounter/spawn overrides, completion flags.
...
- **Map vs. Dungeon Level.** A Dungeon Level adds dungeon-specific metadata around a Map — without it, an authored dungeon would just be N loose Maps with no record of "these go together, in this order, under this name." Levels keep that grouping authoritative.
